...
def setup_driver(version="114.0.5735.90"):
    ...
    try:
        ...
        print(f"ChromeDriverManager failed for version {version}: {e}")
        print("Trying with default ChromeDriverManager...")
...
def scrape_amazon_top_10_playwright(search_term):
    """
    Use Playwright to search Amazon for `search_term` and return lists of:
      • asins   (length 10)
      • titles  (length 10)
      • prices  (length 10)
    """
    ...
    with sync_playwright() as p:
        ...
        for item in results:
            ...
            try:
                ...
                continue
            try:
                price_elem = item.query_selector("span.a-offscreen")
                full_price = price_elem.inner_text().strip() if price_elem else ""
            except:
                full_price = ""
            asins.append(asin)
            titles.append(full_title)
...
async def scrape_amazon_top_10_playwright_async(search_term):
    asins, titles, prices = [], [], []
    async with async_playwright() as p:
        ...
        for item in results:
            ...
            if not asin:
                continue
            try:
                # Try to get the title robustly
                title_elem = await item.query_selector("h2 span")
                full_title = await title_elem.inner_text() if title_elem else ""
                if not full_title:
                    continue
            except:
                continue
            try:
                # Try to get the price robustly
                price_elem = await item.query_selector("span.a-offscreen")
                full_price = await price_elem.inner_text() if price_elem else ""
            except:
                full_price = ""
            asins.append(asin)
            titles.append(full_title)
